...
class MerchantsController extends Controller
{
    ...
    */
    ...
    public function store(MerchantCreateRequest $request)
    {
        ...
        if ($result) {
            return redirect(route('admin.merchants.edit', ['id' => $result->id]))->with('message', 'Successfully created');
        }

        return redirect(route('admin.merchants.index'))->with('message', 'Failed to create');
    }
    ...
    public function destroy($id)
    {
        ...
        if ($result) {
            return redirect(route('admin.merchants.index'))->with('message', 'Successfully deleted');
        }

        return redirect(route('admin.merchants.index'))->with('message', 'Failed to delete');
    }
}
